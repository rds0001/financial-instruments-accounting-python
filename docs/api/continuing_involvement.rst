...
.. autofunction:: financial_accounting_engine.continuing_involvement
...
``examples/analyst_workbook.py`` case ``continuing_involvement`` supplies the arguments and an
independent expected-result assertion. Its function call is::

   import financial_accounting_engine as fae
   actual = fae.continuing_involvement(*args, **kwargs)
   assert check(actual)

Executed pytest ID: ``tests/test_api.py::test_api_example[continuing_involvement]``.
