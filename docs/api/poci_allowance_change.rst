...
.. autofunction:: financial_accounting_engine.poci_allowance_change
...
``current_lifetime_ecl``
   Current nonnegative lifetime expected loss in the initial baseline currency.

``initial_lifetime_ecl``
   Nonnegative initial POCI lifetime ECL baseline, same currency as current ECL.
...
``examples/analyst_workbook.py`` case ``poci_allowance_change`` supplies the arguments and an
independent expected-result assertion. Its function call is::

   import financial_accounting_engine as fae
   actual = fae.poci_allowance_change(*args, **kwargs)
   assert check(actual)

Executed pytest ID: ``tests/test_api.py::test_api_example[poci_allowance_change]``.
