...
.. autofunction:: financial_accounting_engine.get_parameters
...
``examples/analyst_workbook.py`` case ``get_parameters`` supplies the arguments and an
independent expected-result assertion. Its function call is::

   import financial_accounting_engine as fae
   actual = fae.get_parameters(*args, **kwargs)
   assert check(actual)

Executed pytest ID: ``tests/test_api.py::test_api_example[get_parameters]``.
