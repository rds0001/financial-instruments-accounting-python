...
.. autofunction:: financial_accounting_engine.validate_dataset
...
``examples/analyst_workbook.py`` case ``validate_dataset`` supplies the arguments and an
independent expected-result assertion. Its function call is::

   import financial_accounting_engine as fae
   actual = fae.validate_dataset(*args, **kwargs)
   assert check(actual)

Executed pytest ID: ``tests/test_api.py::test_api_example[validate_dataset]``.
