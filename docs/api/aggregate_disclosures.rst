...
.. autofunction:: financial_accounting_engine.aggregate_disclosures
...
``examples/analyst_workbook.py`` case ``aggregate_disclosures`` supplies the arguments and an
independent expected-result assertion. Its function call is::

   import financial_accounting_engine as fae
   actual = fae.aggregate_disclosures(*args, **kwargs)
   assert check(actual)

Executed pytest ID: ``tests/test_api.py::test_api_example[aggregate_disclosures]``.
