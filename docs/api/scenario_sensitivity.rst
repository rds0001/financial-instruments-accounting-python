...
.. autofunction:: financial_accounting_engine.scenario_sensitivity
...
``losses``
   Scenario-ID keyed nonnegative standalone losses, calculated before weighting.
...
``examples/analyst_workbook.py`` case ``scenario_sensitivity`` supplies the arguments and an
independent expected-result assertion. Its function call is::

   import financial_accounting_engine as fae
   actual = fae.scenario_sensitivity(*args, **kwargs)
   assert check(actual)

Executed pytest ID: ``tests/test_api.py::test_api_example[scenario_sensitivity]``.
