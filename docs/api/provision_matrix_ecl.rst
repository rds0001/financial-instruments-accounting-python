...
.. autofunction:: financial_accounting_engine.provision_matrix_ecl
...
``exposure``
   Nonnegative segment exposure in one currency.

``loss_rate``
   Lifetime provision matrix segment rate in [0,1], supplied rather than estimated.
...
``examples/analyst_workbook.py`` case ``provision_matrix_ecl`` supplies the arguments and an
independent expected-result assertion. Its function call is::

   import financial_accounting_engine as fae
   actual = fae.provision_matrix_ecl(*args, **kwargs)
   assert check(actual)

Executed pytest ID: ``tests/test_api.py::test_api_example[provision_matrix_ecl]``.
